...
namespace BankManagement
{
    public class Bank
    {
        ...
        public static List<Account> GetAllAccounts()
        {
            ...
            foreach (Bank userBank in banks)
            {
                if (userBank.accounts.Count > 0)
                {

                    for (int j = 0; j < userBank.accounts.Count; j++)
                    {
                        accounts.Add(userBank.accounts[j]);
                    }
                }
            }
            return accounts;
        ...
        enterBankName: Console.WriteLine("Enter bank name:");
            ...
            Console.WriteLine("-----Welcome to " + bank.BankName.ToUpperInvariant() + " School with an id " + bank.BankId + "--------");
